...
from data_utils import StateEncoding
import torch
...
from droid.droid.misc.server_interface import ServerInterface


# def resize_and_center_crop_torch(image, target_size=84, device="cuda"):
#     # Convert to tensor and move to GPU
#     # image_tensor = torch.tensor(image, dtype=torch.float32, device=device).permute(2, 0, 1) / 255.0  # (H, W, C) -> (C, H, W)
#     image_tensor = torch.tensor(image, dtype=torch.float32, device=device).permute(2, 0, 1) / 255.0  # (H, W, C) -> (C, H, W)
...
#     # Resize using torchvision (fast on GPU)
#     resize = transforms.Resize((new_h, new_w), interpolation=transforms.InterpolationMode.BILINEAR)
#     resized = resize(image_tensor.unsqueeze(0))  # Add batch dim

#     # Center crop
#     crop = transforms.CenterCrop(target_size)
#     cropped = crop(resized)

#     return (cropped.squeeze(0) * 255).byte().permute(1, 2, 0).cpu().numpy()  # Convert back to NumPy
...
class ZedCamera(object):
    ...
    def get_frame(self):
        ...
        frame = deepcopy(self._frame.get_data())[..., :3]  # Remove Depth
        return frame
...
def rotation_6d_to_euler(d6: torch.Tensor) -> torch.Tensor:
    """
    ...
    """
    a1, a2 = d6[..., :3], d6[..., 3:]
    b1 = a1 / torch.linalg.norm(a1, ord=2, axis=-1)
    b2 = a2 - torch.reduce_sum(b1 * a2, axis=-1, keepdims=True) * b1
    b2 = b2 / torch.linalg.norm(b2, ord=2, axis=-1)
    b3 = torch.linalg.cross(b1, b2)
    mat = torch.stack((b1, b2, b3), axis=-2)
    return torch.euler.from_rotation_matrix(mat)
...
class DebugRobotServer(object):
    ...
    def update_command(self, action, action_space="cartesian_position", gripper_action_space="position", blocking=False):
        pass
...
class DroidEnv(gym.Env):
    def __init__(
        ...
        for name, serial_number in cameras.items():
            ...
            image_spaces[name] = gym.spaces.Box(shape=(width,width, 3), dtype=np.uint8, low=0, high=255)
        ...
        if debug:
            ...
            self.server = ServerInterface(ip_address=ip_address)
            # print("CONST 4.1")
    ...
    def _get_observation(self):
        ...
        # print(len(state['joint_velocities']))
        ...
        state = {
            ...
            # StateEncoding.JOINT_VEL:np.array([state["joint_velocities"]], dtype=np.float32),
        }
        ...
        for name, camera in self.cameras.items():
            ...
            if self.transform_images:
                img = image_tools.resize_with_pad_torch(
                        torch.from_numpy(img.copy())[None], 224, 224
                    ).numpy()

            # if self.transform_images:
            #     img = resize_and_center_crop_torch(img, target_size=self.width)
            images[name] = img



        return dict(state=state, robot_state=full_state, image=images)
    ...
    def step(self, action):
        if self.robot_action_space == "cartesian_position":
            ...
            if self.gripper_action_space == "position":
                gripper = action["desired_absolute"][StateEncoding.GRIPPER]
            action = np.concatenate((pos, rot, gripper), axis=-1)
        elif self.robot_action_space == "joint_velocity":
            action=action
            # vel = action["desired_delta"][StateEncoding.JOINT_VEL]
            # gripper = action["desired_absolute"][StateEncoding.GRIPPER]
            # action = np.concatenate((vel, gripper), axis=-1)
        ...
        # print(action)
        # print(self.robot_action_space, self.gripper_action_space)
        action_dict = self.server.update_command(action, action_space=self.robot_action_space, gripper_action_space=self.gripper_action_space, blocking=False)
        ...
        return self._get_observation(), 0.0, False, False, action_dict

    def reset(self, seed=None, options=None, randomize=False):
        super().reset(seed=seed)
        self.server.update_gripper(0, velocity=False, blocking=True)
        if randomize:
            noise = np.random.uniform(low=self.randomize_low, high=self.randomize_high).tolist()
        else:
            noise = None

        # Calling the args EXACTLY like this is extremely important
        self.server.update_joints(self.reset_joints, False, True, noise)
    ...
    def create_action_dict(self, command):
        action_dict = self.server.create_action_dict(command, self.robot_action_space)
        return action_dict
...
if __name__ == "__main__":
      ...
      env = DroidEnv(
          ...
          cameras={"agent_view": "23897859", "wrist": "12391924"},
          resolution=180,
